...
DEFAULT_CONFIG = ROOT / "configs" / "orbench_reasoning_v2.json"
sys.path.insert(0, str(ROOT / "scripts"))
from orbench_multimodel import (  # noqa: E402
    AdaptiveConcurrency,
    RateLimiter,
...
def condition_dir(cfg: dict[str, Any], key: str) -> Path:
    return resolve(cfg["output_dir"]) / "conditions" / key


def concurrency_path(cfg: dict[str, Any], group: str) -> Path:
    return resolve(cfg["output_dir"]) / "concurrency" / f"{group}.json"
...
def smoke_gate(cfg: dict[str, Any]) -> dict[str, Any]:
    ...
    path = resolve(cfg["output_dir"]) / "smoke_gate.json"
    path.parent.mkdir(parents=True, exist_ok=True)
...
def all_condition_data(cfg: dict[str, Any]) -> tuple[dict[str, dict[tuple[str, str, int], dict[str, Any]]], dict[str, list[dict[str, Any]]], dict[str, dict[str, Any]]]:
    ...
    for baseline in cfg["baselines"]:
        outcomes[baseline["key"]], generations[baseline["key"]] = condition_outcomes(resolve(baseline["directory"]))
...
def summarize(cfg: dict[str, Any]) -> None:
    ...
    summary_dir = resolve(cfg["output_dir"]) / "summary"
    ...
    write_artifact_manifest(resolve(cfg["output_dir"]))

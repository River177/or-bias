...
from orbias.core import ArtifactStore, CongestionController
...
def write_json_atomic(path: Path, value: dict[str, Any]) -> None:
    ArtifactStore(path.parent).write_json_atomic(path.name, value)
...
def artifact_root(config: dict[str, Any], config_path: Path) -> Path:
    value = Path(str(config["artifact_root"]))
    return value if value.is_absolute() else configured_artifact_root() / value
